      ...
      >
        <div className="flex items-center justify-between py-3 lg:py-5 lg:block">
          <Link className="flex items-center" to="/">
            <span className="sm:text-2xl font-extrabold pl-1">
          ...
          <div className="lg:hidden flex gap-8">
            {token && (
              <Link to={"/cart"} className="lg:hidden relative text-2xl">
                <span className="absolute text-center text-[10px] text-white -top-3 -right-2 bg-orange-700 rounded-full h-4 w-4 flex items-center justify-center">
                  {totalItem}
                </span>
                <BsCart />
              </Link>
        ...
        >
          <ul className="items-center space-y-6 lg:flex lg:space-x-6 lg:space-y-0">
            ...
            >
              <div className="flex items-center gap-1 px-2 border rounded-lg bg-orange-50 has-[:focus]:bg-transparent has-[:focus]:border-orange-500">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
            ...
            {token ? (
              ...
              <Link
                to={"/login"}
                onClick={() => setState(!state)}
                className="block text-center lg:w-auto w-full px-4 py-2 bg-orange-600 hover:bg-orange-500 rounded text-white"
              >
                Login
              </Link>
            )}
